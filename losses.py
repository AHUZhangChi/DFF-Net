...
class NCC:
    ...
    def ncc(self, Ii, Ji):
        ...
        # ssmi-011
        cc = (2 * cross) / (I_var + J_var)

        # ssmi-111
        '''
        cross_mean=u_I * u_J * win_size
        cross_mean=tf.maximum(cross_mean, self.eps)
        I_mean=u_I * u_I * win_size
        I_mean=tf.maximum(I_mean, self.eps)
        J_mean=u_J * u_J * win_size
        J_mean=tf.maximum(J_mean, self.eps)
        cc = (2*cross_mean/(I_mean+J_mean))*(2*cross /( I_var+ J_var))
        '''
        # return mean cc for each entry in batch
        k=tf.reduce_mean(K.batch_flatten(cc), axis=-1)
        print(k.shape)
        return k
