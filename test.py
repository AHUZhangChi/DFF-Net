...
reg_times = []
dice_means = []
dice_means_globalnet = []
dice_means_localnet = []
with tf.device(device):
    # load model and build nearest-neighbor transfer model
    model = vxm.networks.VxmDense_train9.load(args.model, input_model=None)
    registration_model = model.get_registration_model1()
    ...
    for i in range(len(img_pairs)):
        ...
        '''
        # apply transform
        warped_seg = transform_model.predict([moving_seg, warp[2]])
        warped_seg1 = transform_model.predict([warped_seg, warp[0]])
        warped_seg2 = transform_model.predict([warped_seg1, warp[1]])

        warped_seg=warped_seg.squeeze()
        warped_seg1=warped_seg1.squeeze()
        warped_seg2=warped_seg2.squeeze()
        ...
        # compute volume overlap (dice)
        overlap1 = vxm.py.utils.dice(warped_seg1, fixed_seg, labels=labels)
        dice_means_unet.append(np.mean(overlap1))

        # compute volume overlap (dice)
        overlap2 = vxm.py.utils.dice(warped_seg2, fixed_seg, labels=labels)
        '''
        # apply transform
        warped_seg = transform_model.predict([moving_seg, warp[2]]).squeeze()
        # compute volume overlap (dice)
        overlap = vxm.py.utils.dice(warped_seg, fixed_seg, labels=labels)
        dice_means.append(np.mean(overlap))

        warped_seg1 = transform_model.predict([moving_seg, warp[0]]).squeeze()
        # compute volume overlap (dice)
        overlap1 = vxm.py.utils.dice(warped_seg1, fixed_seg, labels=labels)
        dice_means_globalnet.append(np.mean(overlap1))

        warped_seg2 = transform_model.predict([moving_seg, warp[1]]).squeeze()
        # compute volume overlap (dice)
        overlap2 = vxm.py.utils.dice(warped_seg2, fixed_seg, labels=labels)
        dice_means_localnet.append(np.mean(overlap2))
        print(
            'Pair %d    Reg Time: %.4f    Dice-final: %.4f +/- %.4f    Dice-globalnet: %.4f +/- %.4f    Dice-localnet: %.4f +/- %.4f' % (
                i + 1, reg_time,
                np.mean(overlap),
                np.std(overlap),
                np.mean(overlap1),
...
print('Avg Dice-globalnet: %.4f +/- %.4f' % (np.mean(dice_means_globalnet), np.std(dice_means_globalnet)))

print('Avg Dice-localnet: %.4f +/- %.4f' % (np.mean(dice_means_localnet), np.std(dice_means_localnet)))
