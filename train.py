...
parser.add_argument('--lambda', type=float, dest='lambda_weight', default=1,
...
if args.atlas:
    ...
    generator = vxm.generators.scan_to_atlas1(train_files, atlas,
    ...
    generator = vxm.generators.scan_to_scan1(
...
dec_nf = args.dec if args.dec else [32, 32, 32, 32, 16, 16, 8]

# prepare model checkpoint save path
save_filename = os.path.join(model_dir, '{epoch:04d}.h5')

# build the model----------------------------------------
model = vxm.networks.VxmDense_train9(
    inshape=inshape,
...
# prepare image loss
image_loss_func1 = vxm.losses.NCC().loss
image_loss_func2 = vxm.losses.MSE(args.image_sigma).loss
image_loss_func3 = vxm.losses.NCC().loss
field_loss_func1 = vxm.losses.Grad('l2', loss_mult=args.int_downsize).loss
field_loss_func2 = vxm.losses.Grad('l2', loss_mult=args.int_downsize).loss
field_loss_func3 = vxm.losses.Grad('l2', loss_mult=args.int_downsize).loss
# need two image loss functions if bidirectional

losses = [image_loss_func1, field_loss_func1, image_loss_func2, field_loss_func2, image_loss_func3, field_loss_func3]
weights = [0.4, args.lambda_weight * 0.4, 0.4*100, args.lambda_weight * 0.4, 1, args.lambda_weight]
